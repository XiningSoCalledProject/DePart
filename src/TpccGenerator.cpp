...
// Explicit usings to avoid collision between std::cout and std::count
// (<algorithm> brings std::count into scope, which confuses the compiler
//  when 'using namespace std' is in effect and 'cout' is unqualified).
using std::cout;
using std::endl;
using std::flush;
using std::string;
...
{
   string s = to_string(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count()); // XXX
