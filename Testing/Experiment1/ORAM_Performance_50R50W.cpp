//
// ORAM_Performance_50R50W.cpp
//
// Created by Xining Yuan on 3/1/26.
//
// Workload: Read:Write = 1:1  (Read=50%, Write=50%)
//
...
static constexpr int    WRITE_PCT = 50;
static const std::string WORKLOAD_TAG = "50R50W";

// Physical server cap: FPC never produces more partitions than this.
static constexpr uint32_t NUM_SERVERS = 4;

// Repeated runs per configuration — we take the median to reduce noise.
static constexpr int NUM_REPEATED_RUNS = 5;

// FixedThreshold data-size multiples: data_size = mult × T_records.
static const std::vector<uint32_t> FTH_MULTIPLES = {1, 2, 3, 4, 5};
...
// ############################################################################
//  PHASE 1: ORAM Size Benchmark — Workload: Read:Write = 1:1
// ############################################################################
    ...
    int num_batches)
{
    ...
    for (int batch = 0; batch < num_batches; ++batch) {
        ...
        int read_ops_count  = ops_per_batch * 50 / 100;
...
// ############################################################################
//  PHASE 3: End-to-End — Workload: Read:Write = 1:1
// ############################################################################
    ...
    int EPOCH_MS)
{
    ...
    // ── Step 3: Epoch collection — FIXED WORKLOAD Read:Write = 1:1 ──
    {
        std::lock_guard<std::mutex> lk(queue_mu_);
...
// ############################################################################
//  MAIN — Workload: Read:Write = 1:1
// ############################################################################

int main() {
    std::cout << "\n";
    std::cout << "╔══════════════════════════════════════════════════════════════╗\n";
    std::cout << "║  ORAM Performance — Workload: Read:Write = 1:1                       ║\n";
    std::cout << "║  Read=50%  Write=50%                                          ║\n";
    std::cout << "╚══════════════════════════════════════════════════════════════╝\n\n";
    ...
    std::cout << "Workload: Read:Write = 1:1  (R=" << READ_PCT << "% W=" << WRITE_PCT << "%)\n\n";
    ...
    // X-axis = data_size multiple.  Each mult uses a tiled subset of data.
