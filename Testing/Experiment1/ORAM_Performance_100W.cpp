//
// ORAM_Performance_100W.cpp
//
// Created by Xining Yuan on 3/1/26.
//
// Workload: 100% Write  (Read=0%, Write=100%)
//
...
// Workload config
// ============================================================================
static constexpr int    READ_PCT  = 0;
static constexpr int    WRITE_PCT = 100;
static const std::string WORKLOAD_TAG = "100W";
...
// ############################################################################
//  PHASE 1: ORAM Size Benchmark — Workload: 100% Write
// ############################################################################
    ...
    int num_batches)
{
    ...
    for (int batch = 0; batch < num_batches; ++batch) {
        // 100% WRITE workload
        result.read_batch_ms.push_back(0.0);  // no reads
        ...
        for (int i = 0; i < ops_per_batch; ++i) {
            ...
            update_batch.operations.push_back(
                make_op(OpType::UPDATE, block_id, val, 0, (i == ops_per_batch - 1)));
        }
    ...
    result.avg_read_per_op_ms  = 0.0;
    result.avg_write_per_op_ms = total_write_ms / total_ops;
    result.avg_total_per_op_ms = result.avg_write_per_op_ms;
...
// ############################################################################
//  PHASE 3: End-to-End — Workload: 100% Write
// ############################################################################
    ...
    int EPOCH_MS)
{
    ...
    // ── Step 3: Epoch collection — FIXED WORKLOAD 100% Write ──
    {
        std::lock_guard<std::mutex> lk(queue_mu_);
        ...
        client_threads.emplace_back([&, c]() {
            ...
            for (int t_idx = 0; t_idx < TXNS_PER_CLIENT; t_idx++) {
                ...
                for (int q = 0; q < OPS_PER_TXN; q++) {
                    ...
                    std::string val = "c" + std::to_string(c) + "_upd_"
...
// ############################################################################
//  MAIN — Workload: 100% Write
// ############################################################################

int main() {
    std::cout << "\n";
    std::cout << "╔══════════════════════════════════════════════════════════════╗\n";
    std::cout << "║  ORAM Performance — Workload: 100% Write                             ║\n";
    std::cout << "║  Read=0%  Write=100%                                          ║\n";
    std::cout << "╚══════════════════════════════════════════════════════════════╝\n\n";
    ...
    // OVERHEAD_TOLERANCE = 2.0 so T converges to the largest tested size (500),
    // giving FTH multiples of 500/1000/1500/2000/2500 — matching 100R dataset sizes.
    double OVERHEAD_TOLERANCE  = 2.0;

    // DP epsilon budget allocation
    // FixedPartCount: 70% bin noising, 30% threshold noise
    double fpc_epsilon_1_ratio = 0.7;
    double fpc_epsilon_2_ratio = 0.3;
    // FixedThreshold: threshold is FIXED — ALL budget goes to bin noising.
    ...
    std::cout << "Workload: 100% Write  (R=" << READ_PCT << "% W=" << WRITE_PCT << "%)\n\n";
