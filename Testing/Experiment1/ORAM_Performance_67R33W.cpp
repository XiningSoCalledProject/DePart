//
// ORAM_Performance_67R33W.cpp
//
// Created by Xining Yuan on 3/1/26.
//
// Workload: Read:Write = 2:1  (Read=67%, Write=33%)
//
...
// Workload config
// ============================================================================
static constexpr int    READ_PCT  = 67;
static constexpr int    WRITE_PCT = 33;
static const std::string WORKLOAD_TAG = "67R33W";
...
// ############################################################################
//  PHASE 1: ORAM Size Benchmark — Workload: Read:Write = 2:1
// ############################################################################
    ...
    int num_batches)
{
    ...
    for (int batch = 0; batch < num_batches; ++batch) {
        // Mixed workload: 67% READ, 33% WRITE
        int read_ops_count  = ops_per_batch * 67 / 100;
        int write_ops_count = ops_per_batch - read_ops_count;

        // READ portion
        BatchInfo read_batch;
        read_batch.server_id  = 0;
        read_batch.batch_type = OpType::READ;
        for (int i = 0; i < read_ops_count; ++i) {
            uint32_t block_id = rng() % num_blocks;
            read_batch.operations.push_back(
                make_op(OpType::READ, block_id, "", 0, (i == read_ops_count - 1)));
        }
        ...
        // WRITE portion
        BatchInfo update_batch;
        update_batch.server_id  = 0;
        update_batch.batch_type = OpType::UPDATE;
        for (int i = 0; i < write_ops_count; ++i) {
            uint32_t block_id = rng() % num_blocks;
            std::string val = "bench_upd_" + std::to_string(block_id);
            update_batch.operations.push_back(
                make_op(OpType::UPDATE, block_id, val, 0, (i == write_ops_count - 1)));
        }

        auto w_start = clk::now();
        if (write_ops_count > 0) {
            auto w_result = multi_oram.sendBatchToServer(server, update_batch);
            assert(w_result.success && "UPDATE benchmark batch failed");
        }
        auto w_end = clk::now();
        result.write_batch_ms.push_back(ms_between(w_start, w_end));
    }

    // ── Compute averages ──

    double total_read_ms = 0, total_write_ms = 0;
    for (auto v : result.read_batch_ms)  total_read_ms  += v;
    for (auto v : result.write_batch_ms) total_write_ms += v;

    int read_ops_count  = ops_per_batch * 67 / 100;
    int write_ops_count = ops_per_batch - read_ops_count;
    double total_r = (double)read_ops_count  * num_batches;
    double total_w = (double)write_ops_count * num_batches;

    result.avg_read_per_op_ms  = (total_r > 0) ? (total_read_ms  / total_r) : 0.0;
    result.avg_write_per_op_ms = (total_w > 0) ? (total_write_ms / total_w) : 0.0;
    // Weighted average
    double total_all = total_r + total_w;
    result.avg_total_per_op_ms = (total_all > 0)
        ? ((total_read_ms + total_write_ms) / total_all) : 0.0;
...
// ############################################################################
//  Helper: median of a double vector
// ############################################################################
...
static DiagResult average_diag_results(const std::vector<DiagResult>& runs) {
    if (runs.empty()) throw std::runtime_error("average_diag_results: empty runs");
...
// ############################################################################
//  Helper: get a data subset of exactly `target_size` rows (tiles if needed)
// ############################################################################
    ...
    uint32_t target_size)
{
    ...
    for (uint32_t i = 0; i < target_size; ++i)
        subset.push_back(data[i % data.size()]);
    return subset;
}

// ############################################################################
//  PHASE 3: End-to-End — Workload: Read:Write = 2:1
// ############################################################################
    ...
    int EPOCH_MS)
{
    ...
    // ── Step 3: Epoch collection — FIXED WORKLOAD Read:Write = 2:1 ──
    {
        std::lock_guard<std::mutex> lk(queue_mu_);
        ...
        client_threads.emplace_back([&, c]() {
            ...
            for (int t_idx = 0; t_idx < TXNS_PER_CLIENT; t_idx++) {
                ...
                for (int q = 0; q < OPS_PER_TXN; q++) {
                    ...
                    // 67% READ, 33% WRITE
                    OpType type = ((rng() % 100) < 67)
                        ? OpType::READ : OpType::UPDATE;
                    std::string val = "";
                    if (type == OpType::UPDATE)
                        val = "c" + std::to_string(c) + "_upd_"
...
// ############################################################################
//  MAIN — Workload: Read:Write = 2:1
// ############################################################################

int main() {
    std::cout << "\n";
    std::cout << "╔══════════════════════════════════════════════════════════════╗\n";
    std::cout << "║  ORAM Performance — Workload: Read:Write = 2:1                       ║\n";
    std::cout << "║  Read=67%  Write=33%                                          ║\n";
    std::cout << "╚══════════════════════════════════════════════════════════════╝\n\n";
    ...
    auto resolve_csv_path = []() -> std::string {
        ...
        return dir + "/../../Testing/Unit Tests/test_data_skewed_2500.csv";
    };
    ...
    // OVERHEAD_TOLERANCE = 2.0 (larger than 100R's 1.5) so that T converges to
    // the largest tested size (500), giving FTH multiples of 500/1000/1500/2000/2500.
    double OVERHEAD_TOLERANCE  = 2.0;

    // DP epsilon budget allocation
    // FixedPartCount: split budget — 70% bin noising, 30% threshold noise
    double fpc_epsilon_1_ratio = 0.7;
    double fpc_epsilon_2_ratio = 0.3;
    // FixedThreshold: threshold is FIXED (no noise), ALL budget goes to bin noising.
    double fth_epsilon_1_ratio = 1.0;
    double fth_epsilon_2_ratio = 0.0;
    // Backward-compat aliases for FPC calls
    ...
    // FixedThreshold scaling: data_size = mult × T for each multiple.
    ...
    std::cout << "Workload: Read:Write = 2:1  (R=" << READ_PCT << "% W=" << WRITE_PCT << "%)\n\n";
        ...
        for (uint32_t ns : prescan_hints) {
            for (bool fixed : {true, false}) {
                ...
                auto parts = rep.Repartition_Main(
                    ...
                    epsilon_1_ratio * eps,
                    epsilon_2_ratio * eps,
                    0.0,  // threshold_pub: prescan only
                    fixed, ns, primary_key_col);
    ...
    // ── PHASE 3a: FixedPartCount — vary hint_k, fixed full dataset ────────────
    ...
    // ── PHASE 3b: FixedThreshold — vary data_size = mult × T ─────────────────
